export function createLibraryService({getJson, onChange = () => {}, storage}) {
  ...
  let state = {items: [], search: "", sort: "newest", favorites: false, loading: false, error: "", hasMore: false, nextCursor: null, total: 0};
  try { const saved=JSON.parse(storage?.getItem("localImageChat.studioLibrary.v1"));if(saved)state={...state,search:String(saved.search??""),sort:saved.sort==="oldest"?"oldest":"newest",favorites:saved.favorites===true}; } catch {}
  ...
  return {getSnapshot: snapshot, load, more: () => load({append: true}),
    setQuery(patch) { state = {...state, ...patch, items: [], nextCursor: null, hasMore: false};
      try { storage?.setItem("localImageChat.studioLibrary.v1",JSON.stringify({search:state.search,sort:state.sort,favorites:state.favorites})); } catch {}
      return load(); },
