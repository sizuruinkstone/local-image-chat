...
export function createPromptDock({ onOpenPrompt, onGenerate, onInspector, onModel = onInspector, onPrimary = onInspector, onBrowseLoras = onPrimary, onCancel, previewMode = false }) {
  ...
  const negative = button("Negative", { className: "context-control negative-summary", onClick: () => onOpenPrompt("negative") });
  ...
  const root = element("section", { class: "prompt-dock", "aria-label": "Prompt Dock" }, [
    element("div", { class: "prompt-heading" }, [mode, negative, button("Final Prompt", { className: "context-control", onClick: () => onOpenPrompt("final") })]),
    element("div", { class: "prompt-compose" }, [summaryGrid, rawSummary, generate]),
  ...
  return { root, focus() { mode.focus(); }, render(snapshot, canvasState, view) {
    ...
    if (snapshot?.ready) {
      ...
      negative.querySelector("span").textContent = `Negative${prompt.negativePrompt ? " · 設定済み" : " · 未設定"}`;
