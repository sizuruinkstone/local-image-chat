...
export function createRuntimeController({
  ...
  async function selectRuntime(requestedRuntimeId = elements.runtimeSelect.value) {
  ...
  function renderCheckpointControls() {
    ...
    for (const checkpoint of [selectedCheckpoint, activeCheckpoint]) {
      if (checkpoint && !installedCheckpoints.some((item) => item.title === checkpoint.title)
        && ![...elements.checkpointSelect.options].some((item) => item.value === checkpoint.title)) {
        elements.checkpointSelect.append(new Option(checkpoint.title, checkpoint.title));
      }
    }
  ...
  async function loadCheckpoints(context = runtimeRequestContext()) {
    ...
    try {
      ...
      if (requestToken === catalogToken && isRuntimeContextCurrent(context)) {
        ...
        elements.refreshCheckpointsButton.disabled = getGenerationBusy() || switching || refreshInFlight;
      }
    }
  }
  ...
  async function refreshCheckpoints() {
    ...
    try {
      ...
      if (requestToken === refreshToken && requestCatalogToken === catalogToken
        ...
        elements.checkpointSelect.disabled = !installedCheckpoints.length;
        elements.refreshCheckpointsButton.disabled = getGenerationBusy() || switching;
      }
      if (requestToken === refreshToken && requestCatalogToken === catalogToken
  ...
  const onRefresh = () => void refreshCheckpoints();
