...
catalog[3].registry.category="character";catalog[3].registry.characterTriggerWords="texture-3, gentle light";
catalog[3].registry.outfitPresets=[{id:"linen",name:"Linen outfit",triggerWords:"texture-3, gentle light, linen dress"},{id:"coat",name:"Blue coat",triggerWords:"texture-3, gentle light, blue coat"}];
...
try{
 ...
 assert.equal(await page.getByRole("combobox",{name:"Trigger target section"}).count(),0);
 ...
 const outfit=page.getByRole("combobox",{name:"asset-3 outfit",exact:true});await outfit.selectOption({label:"Linen outfit"});
 assert.equal(await page.getByRole("button",{name:/move (up|down)$/}).count(),0);
 const outfitBox=await outfit.boundingBox(),weightBox=await weight.boundingBox();assert.ok(outfitBox.x+outfitBox.width<=weightBox.x);
 await page.getByRole("button",{name:"asset-0 Disable",exact:true}).click();
 assert.equal(await page.locator(".composition-row").first().locator("strong").textContent(),"asset-0");
 ...
 assert.equal(await page.getByRole("textbox",{name:"追加プロンプト",exact:true}).inputValue(),"");
 for(const label of ["キャラクター","容姿・衣装","画風・品質"]){
  const group=page.getByLabel(`${label} Trigger Words`,{exact:true});
 ...
 const extraGroup=page.getByLabel("追加プロンプト Trigger Words",{exact:true});await extraGroup.locator("summary").click();
 await page.getByRole("textbox",{name:"Negative Prompt",exact:true}).fill("blur");
 assert.notEqual(await extraGroup.getAttribute("open"),null);
 await page.screenshot({path:path.join(output,"section-triggers.png"),animations:"disabled"});

 assert.ok((await page.getByLabel("キャラクター Trigger Words",{exact:true}).textContent()).includes("texture-3"));
 const characterTriggers=page.getByLabel("キャラクター Trigger Words",{exact:true});
 const appearanceTriggers=page.getByLabel("容姿・衣装 Trigger Words",{exact:true});
 await characterTriggers.getByRole("button",{name:"texture-3を無効にする",exact:true}).click();
 await appearanceTriggers.getByRole("button",{name:"linen dressを無効にする",exact:true}).click();
 assert.doesNotMatch(await page.getByRole("textbox",{name:"Final Positive Prompt",exact:true}).inputValue(),/texture-3|linen dress/);
 assert.equal(await page.getByRole("textbox",{name:"キャラクター",exact:true}).inputValue(),"a ceramic teapot");
 await characterTriggers.getByRole("button",{name:"texture-3を有効にする",exact:true}).click();
 await appearanceTriggers.getByRole("button",{name:"linen dressを有効にする",exact:true}).click();
 const final=await page.getByRole("textbox",{name:"Final Positive Prompt",exact:true}).inputValue();assert.ok(final.includes("linen dress"));assert.ok(!final.includes("texture-0"));assert.ok(final.includes(initialPrompt));
 ...
 assert.equal(requests[0].prompt,final);assert.equal(requests[0].negativePrompt,"blur");assert.deepEqual(requests[0].loras.map(l=>l.name),["asset-0","asset-3"]);assert.equal(requests[0].loras[1].weight,0.6);assert.equal(requests[0].loras[0].enabled,false);assert.equal(requests[0].loras[0].triggerWords,"");
 ...
 await open();await find("asset-0");await select("Soft afternoon 01");if(page.viewportSize().width<=700)await page.getByRole("button",{name:"Composition · 2",exact:true}).click();await page.getByRole("combobox",{name:"asset-3 outfit",exact:true}).selectOption({label:"Blue coat"});await close();
 await page.getByRole("button",{name:"Raw · 編集",exact:true}).click();assert.equal(await page.getByRole("textbox",{name:"Final Positive Prompt",exact:true}).inputValue(),"raw ceramic, gentle light, texture-3, blue coat");
