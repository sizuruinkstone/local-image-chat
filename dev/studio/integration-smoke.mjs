...
await page.route("**/outputs/test-*.svg",route=>route.fulfill({contentType:"image/svg+xml",body:svg}));
...
try{
 ...
 await page.getByRole("spinbutton",{name:"Seed",exact:true}).fill("321");await page.getByRole("spinbutton",{name:"Seed",exact:true}).press("Tab");
 ...
 await shot("inspector-draft-1440");await page.getByRole("button",{name:"Close inspector",exact:true}).click();
 await page.getByRole("button",{name:"Final Prompt",exact:true}).click();const final=await page.getByRole("textbox",{name:"Final Positive Prompt",exact:true}).inputValue();await page.keyboard.press("Escape");
 ...
 await shot("integration-generating-1440");status="done";await waitPhase("completed");
 ...
 await page.getByRole("button",{name:"Inspector",exact:true}).click();await page.getByRole("button",{name:"Result Metadata",exact:true}).click();assert.ok((await page.locator(".inspector-result .result-metadata").textContent()).includes("Seed 102"));
 ...
 await page.waitForFunction(()=>document.querySelector(".seed-summary").textContent.includes("102"));await page.getByRole("button",{name:"Close inspector",exact:true}).click();
 ...
 for(const [width,height]of [[390,844],[430,932]]){
  ...
  await page.getByRole("button",{name:"Inspector",exact:true}).click();await shot(`inspector-${width}`);await page.keyboard.press("Escape");
 ...
 for(const [width,height,canvasHeight,dockHeight]of [[1280,900,624,178],[1440,900,624,178],[1920,1080,782,196],[390,844,496,202],[430,932,584,202]]){
  ...
  assert.equal(size.canvas,canvasHeight);assert.equal(size.dock,dockHeight);measurements.push(size);
 }
