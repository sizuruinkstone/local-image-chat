...
- 生成操作バーの生成枚数ステッパー（1〜4枚・不正値では生成しない）と`結合結果を確認`の移動
- サムネイル付きLoRA選択（フォルダ・分類・検索・Favorite絞り込み・遅延読み込み）
- キャラクター選択・衣装選択（`public/preset-catalog.js`）と、関連LoRA／トリガーワード連動
- LoRA Favorite（レジストリ保存）と画像Favorite（履歴）の分離、Favorite表示の即時同期

## v2.21で見送った項目

### キャラ・衣装の手動登録

- 現状: キャラ・衣装の候補は既存のLoRA分類・プロフィール・Civitai衣装プリセットから作る。
  「このLoRAはキャラ」「この組み合わせを衣装として登録」という手動作成UIは無い。
  分類を変えたい場合は設定画面のLoRA管理で`分類`と`詳細分類`を編集する。
- 次にやるなら: `data/`へ`presets.json`を作り、`preset-catalog.js`の一覧へマージする。
  既存のLoRAメタデータは書き換えない方針は維持する。

### キャラ・衣装専用サムネイル

- 現状: サムネイルは関連LoRAのプレビュー画像を流用する（衣装ごとの画像は持たない）。
- 次にやるなら: 上の`presets.json`へ`thumbnailUrl`を持たせ、履歴の生成画像から選べるようにする。
