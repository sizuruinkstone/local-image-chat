...
現在の正式版は`v3.0.0`です。生成画面は左カラムで編集、中央で生成画像、右カラムで履歴と読み取り専用メタデータを扱う3カラム構成です。ギャラリーはサムネイル表示と20件ページングを使用し、設定画面とギャラリー画面を整理しています。LoRAはキャラクター特徴と衣装プリセットを分離し、生成完了時のDiscord通知にも対応しています。
...
- `比較実験へ送る`を下部バーから外し、生成設定内の折りたたみ`比較実験`（初期は閉じる）へ移動
- 生成ボタンは54px、`−`／`＋`は46px。下部ナビの直上へ置き、本文には固定バー分の余白を確保する

## v2.21の主な機能

- 生成操作バーへ生成枚数のステッパー（`−` / 直接入力 / `＋`、1〜4枚）を追加。不正値のままでは生成しない
- `結合結果を確認`は`分割入力 / Raw Prompt`の切り替え付近へ移動し、操作バーは`比較実験へ送る`と`画像を生成`だけに
- LoRA追加をサムネイル付きの選択画面へ刷新（フォルダ・分類・検索・★Favorite絞り込み・追加済み表示・遅延読み込み）
- LoRAの★Favoriteは`data/lora-registry.json`へ保存し、画像のFavoriteとは別データとして扱う
- キャラクター選択・衣装選択を追加。サムネイル一覧から選ぶと、対応する項目へタグを反映し、関連LoRAとトリガーワードも既存仕様どおり連動する
- キャラと衣装は独立。片方を変えても、もう片方の入力は消えない（置き換え / 追加を選択可能）
- 画像Favoriteをギャラリーカード右上の★・履歴詳細・最新結果パネル・候補カードから操作でき、どこから変えても即時同期する

## v2.20の主な機能

- 画面を`生成` / `ギャラリー` / `比較` / `設定`の4つへ整理。機能を増やしてもトップレベルのタブは増やさない
- PCの生成画面は「左ナビ・中央入力・右に最新の生成結果（sticky）」の3カラム
- 生成ボタンは中央下部の追従バーへ（`結合結果を確認` / `比較実験へ送る` / `画像を生成`）。生成中は同じ位置に進捗と中止を表示
- スマホは1カラム＋下部ナビ。生成ボタンは下部ナビのすぐ上へ固定
- 構造化プロンプトは各項目アコーディオン。閉じていても`3タグ` / `空`と先頭の内容が分かる（Negative Promptも同じ形式）
- Checkpointを生成設定の先頭へ統合（選択・再読込）。管理（既定値・LoRAセット）は設定画面へ
- SamplerとSchedulerは候補一覧から選択（お気に入り・最近使用・検索）。`DPM++ 2M SDE / Karras`などのプリセットも用意
- 使用頻度の低い設定（Hires fix・Noise schedule・自動リカバリ）は「詳細設定」へ折りたたみ
- 生成画面のLoRAは「使用中LoRA」だけを操作（追加・お気に入りから追加・Weight・ON/OFF・削除）。所有LoRAの管理は設定画面へ
- ギャラリーで履歴を統合表示し、すべて / Favorite / 通常生成 / 比較実験 / Checkpoint / LoRA / 生成日時 / 検索で絞り込める
- **生成中でも画面を自由に移動できる**。生成はサーバー側のジョブとして進み、右上のキュー表示から進捗を確認できる。完成した画像はギャラリーへ自動反映され、生成画面へ戻れば結果と設定はそのまま
...
同じリポジトリを同じポートで二重起動すると、後から起動したプロセスだけが拒否されます。別のポートを指定した起動は許可されます。実行中のPID・起動日時・待受先は、設定画面の「アプリ情報」で確認できます。画面に再起動が必要と表示された場合は、サーバーと静的ファイルのバージョンが一致していないため、既存のサーバーを終了してから再起動してください。
...
トップレベルの画面は4つです。左（スマホは下部）のナビで切り替えます。URLの`#gallery`などでも直接開けます。

| 画面 | 役割 |
| --- | --- |
| 生成 | 入力・LoRA・生成設定・実行・最新の生成結果 |
| ギャラリー | 生成画像と履歴の閲覧・絞り込み・詳細・再利用 |
| 比較 | 比較実験の作成・キュー・結果 |
| 設定 | Checkpoint管理・LoRA管理・Civitai・Discord・スマホ接続・アップデート |

**生成中でも画面を移動できます。** 生成はサーバー側のジョブとして進むため、ギャラリーや設定へ移動しても中断・再実行されません。進捗はヘッダー右上のキュー表示と画面上部の進捗バーで確認でき、完成した画像はギャラリーへ自動で反映されます。生成画面へ戻れば、候補・設定・プロンプトはそのまま残っています。

### 生成画面（PC）

左にナビ、中央に入力（構造化プロンプト → LoRA → 生成設定 → 補助設定 → 実行）、右に最新の生成結果を置きます。右の結果パネルはスクロールに追従します。

中央の下部には操作バーが追従表示されます。置くのは生成枚数と主ボタンだけです。

```text
生成枚数  [−] [1] [+]    [画像を生成（1枚）]
```

生成中は同じ位置が進捗表示へ変わり、`中止`を押せます。スマホでは下部ナビのすぐ上に同じ1行で表示し（幅が足りないときだけ生成ボタンが次の行へ回ります）、`結合結果を確認`は`分割入力 / Raw Prompt`の切り替え付近、比較実験は生成設定内の折りたたみへ置いています。

### 生成設定

Checkpointは生成設定の先頭にあります（選択と`再読込`）。既定値・自動適用・LoRAセットの管理は設定画面の`Checkpoint管理`です。

SamplerとSchedulerは現在値を押すと候補一覧が開き、`★ お気に入り` / `最近使用` / `すべて` / 検索から選べます。★で並び順を固定でき、選んだ値は最近使用へ残ります。よく使う組み合わせはプリセットから1タップで設定できます。

```text
[DPM++ 2M SDE / Karras] [DPM++ 2M / Karras] [Euler a / Automatic]
```

片方だけ変更したときに、もう片方を自動で変えることはありません（プリセットを押したときだけ両方を変えます）。Hires fix・Noise schedule・自動リカバリは`詳細設定`へ、比較実験の入り口は`比較実験`へ折りたたんでいます（どちらも初期状態は閉じています）。

`比較実験`を開くと、比較する項目と比較候補を入力して`現在の設定を比較へ追加`を押せます。値は比較画面のフォームへそのまま渡り、比較の実行と状態管理は従来どおり比較画面が持ちます。

### キャラクター・衣装のクイック選択

分割入力の上にある`キャラを選択` / `衣装を選択`から、サムネイル付きの一覧を開けます。

- **キャラクター**: 分類が`キャラクター`のLoRAと、複数キャラを含むプロフィール（作品まとめ系LoRA）の各キャラが候補です。選ぶと`キャラクター`欄へタグを反映し、関連LoRAを生成フォームへ追加します
- **衣装**: Civitaiから取り込んだ衣装プリセット、キャラLoRAの追加衣装、分類が`衣装`のLoRAが候補です。選ぶと`容姿・衣装`欄へ反映します
- どちらも`現在の内容を置き換える` / `現在の内容へ追加する`を選べます。**反映するのは対象の1項目だけ**で、他の欄（キャラ↔衣装を含む）は変更しません
- 候補が無い場合は、どこから候補を作るかをメッセージで案内します（誤分類はしません）

### 生成画面のLoRA

生成画面では「今回使うLoRA」だけを扱います。`LoRAを追加` / `Civitai URLから追加`と、使用中LoRAのWeight・ON/OFF・削除です。

`LoRAを追加`はサムネイル付きの選択画面を開きます。フォルダ（件数付き）で絞り込み、名前・表示名・トリガーワード・フォルダ・分類を対象に検索できます（選択中のフォルダ内だけを検索）。カード右上の★はLoRAのお気に入りで、`[すべて] [★ Favorite]`で絞り込めます。サムネイルはCivitai登録時のプレビュー画像を使い、無い場合はプレースホルダーを表示します（大量のLoRAでも重くならないよう遅延読み込み）。★は`data/lora-registry.json`へ保存され、**画像のFavoriteとは別データ**です。トリガーワードのWeightは対応する構造化プロンプト項目の直下で調整します。所有LoRAの分類・トリガーワード・既定Weightなどの管理は設定画面の`LoRA`です（設定画面の値は初期値で、生成画面で今回だけ変えた値はマスタへ反映しません）。

## 基本的な使い方

1. 日本語で生成内容を入力する（Promptを直接書く場合は空でもかまいません）。
2. 生成設定の`生成枚数`を`1〜4枚`から選び、`画像を生成`を押す。
3. 候補をクリックして1枚選ぶ。
...
生成した画像は`ギャラリー`画面へ自動追加されます（生成履歴と同じ場所です）。Favoriteはカード右上の★・履歴詳細・最新結果パネル・候補カードのどこからでも切り替えられ、状態は即座に共有されます。上部の絞り込みで`すべて` / `Favorite` / `通常生成` / `比較実験`、Checkpoint・LoRA・生成日時・キーワード検索を組み合わせられます。絞り込みは表示だけを変え、履歴データには手を加えません。保存先は次のとおりです。
...
設定画面の`設定 → 一般 → 画像の保存場所`へ、サーバーPC上の新しい絶対パスを入力し、`変更内容を確認`してから`再起動時の移行を予約`を押します。予約中も現在のサーバーは旧保存先を使い続け、適用にはサーバーの通常終了と再起動が必要です。再起動前に既存の画像本体、参照画像、マスク、`thumbnails/`、`favorite/`などを新保存先へコピーして検証します。

設定は`data/storage-settings.json`（Git管理外）へ保存します。`LOCAL_IMAGE_CHAT_OUTPUT_DIR`が最優先で、設定されている場合は画面から変更できません。`LOCAL_IMAGE_CHAT_FAVORITES_DIR`を設定している場合、Favorite保存先はoutput変更に追従しません。空またはLocal Image Chat専用marker（`.local-image-chat-output.json`）付きの既存フォルダだけを移行先にでき、空でない未管理フォルダ・アプリ管理用フォルダ・UNCパスは指定できません。

移行はコピー方式で、旧保存先を自動削除しません。同名で内容が異なるファイルがある場合は上書きせず失敗として旧保存先で起動します。失敗時は部分コピーを自動削除しないため、空き容量や移行先を確認して再予約するか、予約を解除してください。移行成功後も旧保存先はバックアップとして一定期間保持し、内容を確認してから手動で整理してください。
...
手動でバックアップしたい場合は、`data/`フォルダ（`history.json`・`lora-registry.json`・`experiments.json`・`checkpoint-lora-sets.json`・`storage-settings.json`）と`outputs/`をコピーしてください。保存先移行を予約している場合は、現在の保存先と移行先の両方を確認できるまで保持してください。
