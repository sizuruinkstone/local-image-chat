...
- スマホ対応: 画面幅320〜430pxで1カラム表示。横スクロールなしでプロンプト編集・LoRA設定・ギャラリー・履歴詳細・比較・設定を操作できる
- 用途別プロンプトはアコーディオン化。スマホでは中身のある項目とキャラクターだけを開き、見出しに内容のプレビューを出す
- タップしやすいサイズへ調整（主要ボタン44px前後、`×`などのアイコンも34px以上）。入力欄は16pxでiOSの自動ズームを防止
- 主要操作（`プロンプトだけ作る` / `候補を生成`）をスマホでは画面下部へ吸着表示
- 同じLANのスマホからアクセスできる起動方法を追加（`start-lan.bat` / `LOCAL_IMAGE_CHAT_HOST=0.0.0.0`）。ReForgeはPC内の127.0.0.1のままで公開しない
- PWA対応: ホーム画面へ追加して`standalone`で起動できる（Manifest・アイコン・テーマカラー・セーフエリア対応）
...
画面は幅320〜430pxでも1カラムで操作できます。用途別プロンプトはアコーディオンになり、スマホでは中身のある項目と`キャラクター`だけが開いた状態で始まります。`プロンプトだけ作る` / `候補を生成`は画面下部へ吸着し、ギャラリーは2列で表示します。

### 同じLANのスマホからPCのアプリへ接続する

1. PCで`start-lan.bat`をダブルクリックする（`start.bat`はPC内専用のままです）。
2. 起動ログに出るURLをスマホのブラウザで開く。

```text
Local Image Chat v2.19.0
  http://127.0.0.1:3030
  http://192.168.10.101:3030（同じネットワークの端末から）
  ⚠ ログイン機能はありません。信頼できるLAN・Tailscale内だけで使用してください
```

待ち受け先は環境変数でも指定できます。既定は`127.0.0.1`（PC内のみ）です。

| 変数 | 既定 | 説明 |
| --- | --- | --- |
| `LOCAL_IMAGE_CHAT_HOST` | `127.0.0.1` | `0.0.0.0`にすると同じネットワークの端末から接続できる |
| `LOCAL_IMAGE_CHAT_PORT` | `config.json`の`port`（3030） | 待ち受けポート |

`config.local.json`へ`{ "host": "0.0.0.0", "port": 3030 }`と書いても同じです（環境変数が優先）。

ReForgeとOllamaへの接続は、この変更後もサーバー側から`127.0.0.1:7860` / `127.0.0.1:11434`へ行います。スマホがReForgeへ直接つなぐことはなく、生成リクエストは必ずこのアプリのバックエンドを経由します。

```text
スマートフォン → Local Image Chat（PC上のバックエンド） → ReForge API（127.0.0.1:7860）
```

### Windows側で必要な設定

- **ファイアウォール**: 初回起動時に「Node.js JavaScript Runtime」の通信許可ダイアログが出たら、**プライベートネットワーク**にチェックを入れて許可します。
- ダイアログを閉じてしまった場合は、管理者権限のPowerShellで受信規則を追加します（ポートは使用中のものへ合わせてください）。

```powershell
New-NetFirewallRule -DisplayName "Local Image Chat (LAN)" -Direction Inbound -Action Allow -Protocol TCP -LocalPort 3030 -Profile Private
```

- 規則を消すときは `Remove-NetFirewallRule -DisplayName "Local Image Chat (LAN)"` です。
- PCのIPアドレスは`ipconfig`の「IPv4 アドレス」で確認できます（起動ログにも出ます）。
- **ReForgeのポート(7860)は開けないでください。** アプリ経由でのみ使用します。

### 外出先から使う（Tailscale）

ルーターのポート開放は行いません。Tailscaleなどのプライベートネットワーク経由で接続します。

1. PCとスマホの両方にTailscaleを入れ、同じアカウントでログインする。
2. PCで`start-lan.bat`を起動する（`LOCAL_IMAGE_CHAT_HOST=0.0.0.0`）。
3. スマホのブラウザで`http://<PCのTailscale IP>:3030`（例: `http://100.101.102.103:3030`）を開く。MagicDNSを使っている場合は`http://<マシン名>:3030`でも開けます。

アプリ側はホスト名・IPを固定していないため（すべて相対URLで通信します）、アクセス元のアドレスが変わってもそのまま動作します。進捗は同一オリジンへのHTTPポーリングで取得するので、WebSocketの追加設定は不要です。
...
アプリ名`ImageChat`、`display: standalone`で、ブラウザのUIなしで起動します。オフラインでは生成できないため、Service Workerはキャッシュを持ちません（更新後に古い画面が残りません）。
...
- **このアプリにログイン機能はありません。** `LOCAL_IMAGE_CHAT_HOST=0.0.0.0`で起動している間は、同じネットワークに入れる端末なら誰でも生成・履歴閲覧・設定変更ができます。信頼できる自宅LANかTailscale内だけで使ってください。使い終わったら`start.bat`（PC内のみ）へ戻すのが安全です。
- ルーターのポート開放（インターネットへの直接公開）は行わないでください。外出先からはTailscaleなどのプライベートネットワークを使います。
- ReForge・Ollamaのポートは外部へ公開しません。アプリのバックエンドだけが`127.0.0.1`経由で呼び出します。
- APIは同一オリジン専用です（CORSヘッダーを付けていないため、他サイトのJavaScriptからは呼び出せません）。この設定は広げないでください。
- Civitai・GitHubのトークンはブラウザのsessionStorageに置き、リクエストごとにサーバーへ渡すだけです。DiscordのWebhook URLはサーバー内（`data/discord-settings.json`・環境変数・`config.local.json`）にのみ保持し、画面やAPIレスポンスへは返しません。
- LAN公開中は、フォルダを開く操作やアプリ更新（`/api/update/apply`）も同じネットワークから実行できてしまいます。この点でも接続元を限定してください。
