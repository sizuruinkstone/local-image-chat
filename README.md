...
同時にその画像を`outputs/`から`favorites/`へ複製するので、気に入った画像だけをフォルダで見返せます。👍を外すと`favorites/`側の複製は削除されます（`outputs/`の元画像と履歴は残ります）。
サーバー起動時に、既に👍が付いている画像で`favorites/`に無いものを自動で複製します。
