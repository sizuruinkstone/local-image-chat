...
生成画面では「今回使うLoRA」だけを扱います。`LoRA追加` / `お気に入りから追加` / `Civitai URLから追加`と、使用中LoRAのWeight・ON/OFF・削除です。トリガーワードのWeightは対応する構造化プロンプト項目の直下で調整します。所有LoRAの分類・トリガーワード・既定Weightなどの管理は設定画面の`LoRA`です（設定画面の値は初期値で、生成画面で今回だけ変えた値はマスタへ反映しません）。
...
生成した画像は`ギャラリー`画面へ自動追加されます（生成履歴と同じ場所です）。上部の絞り込みで`すべて` / `Favorite` / `通常生成` / `比較実験`、Checkpoint・LoRA・生成日時・キーワード検索を組み合わせられます。絞り込みは表示だけを変え、履歴データには手を加えません。保存先は次のとおりです。
