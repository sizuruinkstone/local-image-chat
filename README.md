...
中央の下部には操作バーが追従表示されます。

```text
[結合結果を確認] [比較実験へ送る] [画像を生成（1枚）]
```

生成中は同じ位置が進捗表示へ変わり、`中止`を押せます。スマホでは下部ナビのすぐ上に、横幅いっぱいの`画像を生成`が表示されます。
...
片方だけ変更したときに、もう片方を自動で変えることはありません（プリセットを押したときだけ両方を変えます）。Hires fix・Noise schedule・自動リカバリは`詳細設定`へ折りたたんでいます。
