...
test("ギャラリーは20件ページング・サムネイル・遅延読込を使う", async () => {
  const [app, historyController, compare, html] = await Promise.all([
  ...
  assert.match(historyController, /const HISTORY_PAGE_SIZE = 20/);
  assert.doesNotMatch(`${app}\n${historyController}`, /\/api\/history\?limit=200/);
